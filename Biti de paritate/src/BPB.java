...
class Hamming {
    public static void main(String args[]) {
        ...
        for (int i = 0; i < n; i++) {
            System.out.println("Intoduceti bitul nr " + (i + 1) + ":");
        ...
        System.out.println("Mesajul generat este:");
        ...
        System.out.println("Introduceti positia bitului pentru care se va face verificarea in momentul cand mesajul ajunge la destinatar (0 daca nu exista eroare)");
        int error = scan.nextInt();
        if (error != 0) {
            b[error - 1] = (b[error - 1] + 1) % 2;
        }
        System.out.println("Mesajul trimis este:");
        for (int i = 0; i < b.length; i++) {
            System.out.print(b[b.length - i - 1]);
        }
        System.out.println();
    ...
    static void receive(int a[], int parity_count) {
        ...
        if (error_location != 0) {
            System.out.println("Eroarea este localizata la " + error_location + ".");
            a[error_location - 1] = (a[error_location - 1] + 1) % 2;
            System.out.println("Mesajul corect este:");
            for (int i = 0; i < a.length; i++) {
                System.out.print(a[a.length - i - 1]);
            }
            System.out.println();
        } else {
            System.out.println("Nu exista erori in mesajul primit");
        }

        // Finally, we shall extract the original data from the received (and corrected) code:
        System.out.println("Mesajul original a fost trimis:");
